#!/usr/bin/env python3
"""
predict_mutation_v2.py - MODIFIED FOR LOCAL CONTEXT
...
BEFORE (Full sequence):
  - 1 mutation = 0.03% change
  - Feature vectors 99.97% identical
  - SVM can't distinguish mutations

AFTER (±50 bp window):
  - 1 mutation in 100 bp = 1% change
  - 98 k-mers total (not 11,607)
  - ~3% of k-mers change
  - SVM can better distinguish mutations
  - 100x improvement in signal
"""
...
class PredictionPipeline:
    ...
    def predict(self, position, base):
        ...
        pred_proba = self.svm_model.decision_function(vector)[0]

        # Decode class
        class_name = self.label_encoder.inverse_transform([pred_class])[0]

        # Calculate confidence (convert decision function to probability)
        confidence = 1.0 / (1.0 + np.exp(-pred_proba))
